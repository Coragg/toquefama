...
fn generation_of_turns_in_the_game(games: i8, random_numbers: String, quantity: i8) -> (i8, i8) {
    ...
    loop {
        ...
        let result_party: i8 =
            check_user_response(random_numbers.clone(), ask_to_user, quantity.clone());
...
fn check_the_number(
    message_to_user: &str,
    message_in_case_of_error: &str,
    first_conditional: i8,
    second_conditional: i8,
) -> i8 {
...
fn game() -> (i8, i8) {
    let quantity_numbers: i8 = check_the_number(
        "\nHow many digits you wanna play (3 to 9)? ",
        "Enter again, how many digits do you wanna play(3 to 9)? ",
        3,
        9,
    );
    ...
    let how_many_games: i8 = check_the_number(
        "How many games do you want to play(2 to 30)? ",
        "How many games do you want to play (2 to 30)? ",
        2,
        30,
    );

    let get_resul_party: (i8, i8) =
        generation_of_turns_in_the_game(how_many_games, random_number_for_user, quantity_numbers);
    get_resul_party
}

fn start_match(match_information: &mut DataParty) -> () {
    let match_play: (i8, i8) = game();
    if match_play.0 == -1 {
        match_information.lose += 1;
    } else if match_play.0 == 1 {
        match_information.win += 1;
        if match_information.best_party > match_play.1 {
            match_information.best_party = match_play.1;
        }
    }
    match_information.parties += 1;
}

fn menu(user_name: String) -> () {
    let mut matches_played: DataParty = DataParty {
        win: 0,
        lose: 0,
        parties: 0,
        best_party: 30,
    ...
    loop {
        let choose_play: String = input_string("Do you wanna play a match? y/n: ");
        if choose_play.to_lowercase() == "y" || choose_play.to_lowercase() == "yes" {
            start_match(&mut matches_played);
        } else if choose_play.to_lowercase() == "n" || choose_play.to_lowercase() == "no" {
            show_stadistics(matches_played);
            println!("Good bye {}, it was a placer play with you", user_name);
            process::exit(0);
        } else {
            println!("Please, you only have to write yes/y or no/n.")
        }
    }
}
